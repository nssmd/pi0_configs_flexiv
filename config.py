...
class LeRobotFlexivDataConfig(DataConfigFactory):
    """Config for loading Flexiv data from a LeRobot dataset."""

    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        # 重新打包转换，将LeRobot数据集中的键名映射为训练脚本期望的键名
        # 这个转换只应用于数据集数据，不用于推理
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransform(
        ...
        # 数据转换，应用于数据集数据和推理
        # 使用您创建的FlexivInputs和FlexivOutputs类
        data_transforms = _transforms.Group(
        ...
        # 如果您的数据集使用绝对动作（而不是相对动作），可以取消下面的注释
        # 将动作转换为相对动作（相对于序列中的第一个状态）
        # 在下面的例子中，我们将前9个动作（关节）转换为相对动作，
        # 保留第10个动作（夹爪）为绝对动作
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        data=LeRobotFlexivDataConfig(
            repo_id="finetune", 
            base_config=DataConfig(
                local_files_only=True,  # 对于本地数据集设为True
    ...
    TrainConfig(
        name="pi0_flexiv_low_mem_finetune",
        # 这是加载pi0模型进行LoRA微调
        model=pi0.Pi0Config(paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora"),
        data=LeRobotFlexivDataConfig(
            repo_id="finetune",  
